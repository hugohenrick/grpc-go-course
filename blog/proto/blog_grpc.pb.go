...
type BlogServiceClient interface {
	...
	ListBlogId(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (BlogService_ListBlogIdClient, error)
}
...
func (c *blogServiceClient) ListBlogId(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (BlogService_ListBlogIdClient, error) {
	stream, err := c.cc.NewStream(ctx, &BlogService_ServiceDesc.Streams[0], "/blog.BlogService/ListBlogId", opts...)
	if err != nil {
		return nil, err
	}
	x := &blogServiceListBlogIdClient{stream}
...
type BlogService_ListBlogIdClient interface {
	Recv() (*Blog, error)
	grpc.ClientStream
}

type blogServiceListBlogIdClient struct {
	grpc.ClientStream
}

func (x *blogServiceListBlogIdClient) Recv() (*Blog, error) {
	m := new(Blog)
...
type BlogServiceServer interface {
	...
	ListBlogId(*emptypb.Empty, BlogService_ListBlogIdServer) error
...
func (UnimplementedBlogServiceServer) ListBlogId(*emptypb.Empty, BlogService_ListBlogIdServer) error {
	return status.Errorf(codes.Unimplemented, "method ListBlogId not implemented")
}
...
func _BlogService_ListBlogId_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(emptypb.Empty)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BlogServiceServer).ListBlogId(m, &blogServiceListBlogIdServer{stream})
}

type BlogService_ListBlogIdServer interface {
	Send(*Blog) error
	grpc.ServerStream
}

type blogServiceListBlogIdServer struct {
	grpc.ServerStream
}

func (x *blogServiceListBlogIdServer) Send(m *Blog) error {
...
var BlogService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "ListBlogId",
			Handler:       _BlogService_ListBlogId_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "blog.proto",
